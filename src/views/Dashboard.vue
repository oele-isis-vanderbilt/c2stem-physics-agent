        ...
        <div class="card-body">
          ...
          >
            <button
              ...
              class="nav-link"
              :class="{
                'websocket-connected': webSocketStatus,
                'websocket-disconnected': !webSocketStatus,
              }"
              id="v-pills-home-tab"
              data-bs-toggle="pill"
...
export default {
  ...
  methods: {
    ...
    async getUserStatus(username) {
      let response = await Logs.getUserStatus();
      if (response) {
        let userData = response.data.users;
        this.userState = userData.filter((user) => user.username === username);
        this.dataVisible = true;
        this.webSocketStatus = this.userState[0]?.websocket_connected || false;
      }
    },
  },
  async mounted() {
