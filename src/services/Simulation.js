...
export default {
    ...
    if (user) {
      try {
        let response = await axios.get(
          "https://physics.c2stem.org/api/getProjectList",
  ...
  async getProjectListByUser(username) {
    if (username) {
      try {
        let response = await axios.get(
          "https://physics.c2stem.org/api/projects/" + username,
          { withCredentials: true }
        );
  ...
  async getSharedProjectList() {
    ...
    if (user) {
      try {
        let response = await axios.get(
          "https://physics.c2stem.org/api/getSharedProjectList",
          { withCredentials: true }
        );
        const pl = response.data.split(" ");
        pl.forEach((element) => {
    ...
    if (user) {
      try {
        response = await axios.post(
          "https://physics.c2stem.org/api/deleteProject",
